...
const animationTimeline = () => {
  ...
  tl.to(".container", 0.6, { visibility: "visible" })
    ...
    .to(".idea-1", 0.7, ideaTextTransLeave, "+=12")
    ...
    .staggerFrom(".wish-hbd span", 0.4, { opacity: 0, y: -50, rotation: 150, skewX: "30deg", ease: Elastic.easeOut.config(1, 0.5) }, 0.03)
    .staggerFromTo(".wish-hbd span", 0.4, { scale: 1.4, rotationY: 150 }, { scale: 1, rotationY: 0, color: "#ff69b4", ease: Expo.easeOut }, 0.03, "party")
      ...
      if (btn) {
        ...
        btn.addEventListener("click", function () {
          Swal.fire({
            ...
            html: '<strong>Kalo aku jadi ur boy, kamu mau ga jadi my girl? 😆</strong>',
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'Mauu Bangettt! 😍',
            cancelButtonText: 'Engga ah! 😢'
          }).then((result) => {
            const phoneNumber = "6281285026909";
            if (result.isConfirmed) {
              Swal.fire('Yeyyy!', 'Lelll?!?!?! Cihuyyyy❤', 'success');
              const message = encodeURIComponent("Aku uda jawab 'Mauu Bangettt! 😍' hehe 😚");
              window.open(`https://wa.me/${phoneNumber}?text=${message}`, '_blank');
            } else {
              Swal.fire('Yahhh:(', 'Sediihhhhh 😔', 'error');
              const message = encodeURIComponent("Aku jawab 'Engga ah! 😢' maaf yaa 😔");
              window.open(`https://wa.me/${phoneNumber}?text=${message}`, '_blank');
            }
          });
        });
